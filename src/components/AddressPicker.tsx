 ...
 * default sourced from the profile, `__temp__` is an unsaved one-off address.
 */
...
/**
 * Reusable address-book picker: lists the customer's saved addresses (plus their
 * profile default), lets them add a new one, and reports the current selection via
 * `onChange`. Shared by the post-approval delivery step and the vault re-order flow.
 */
const AddressPicker = ({ onChange }: { onChange: (addr: SavedAddress | null) => void }) => {
  const { user } = useAuth();
  ...
  const select = useCallback((addr: SavedAddress | null) => {
  ...
  const loadData = useCallback(async () => {
    ...
    else { select(null); setShowNewForm(true); }
  ...
  if (loading) return (
    <div className="py-10 text-center">
      <div className="animate-spin w-7 h-7 border-4 border-primary border-t-transparent rounded-full mx-auto" />
    </div>
  );
