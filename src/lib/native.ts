...
// Match the native status bar to the current theme: light app → dark icons on white;
// dark app → light icons on the dark background. No-op on the web. Called at startup and
// whenever the in-app dark-mode toggle flips.
export async function applyStatusBarTheme(isDark: boolean): Promise<void> {
  ...
  try {
    ...
    if (Capacitor.getPlatform() === 'android') {
      await StatusBar.setBackgroundColor({ color: isDark ? '#0e131c' : '#FFFFFF' });
    }
